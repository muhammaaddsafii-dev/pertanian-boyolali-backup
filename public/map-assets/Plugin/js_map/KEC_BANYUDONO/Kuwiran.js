const subsidiKuwiranLayer = L.featureGroup();
const nonsubsidiKuwiranLayer = L.featureGroup();

const DesaKuwiran = L.geoJson(null, {
  style: function (feature) {
    const baseStyle = {
      pane: "pane_DesaKuwiran",
      opacity: 1,
      weight: 1,
      fillOpacity: 0.6,
      fill: true,
  ...
  onEachFeature: function (feature, layer) {
    ...
    layer.on({
      ...
      mouseout: function () {
        DesaKuwiran.resetStyle(layer);
      },
    });

    if (feature.properties.SB_PPK === "SUBSIDI") {
      subsidiKuwiranLayer.addLayer(layer);
    } else if (feature.properties.SB_PPK === "NON_SUBSIDI") {
      nonsubsidiKuwiranLayer.addLayer(layer);
    }
  },
});

map.createPane("pane_DesaKuwiran");
Object.assign(map.getPane("pane_DesaKuwiran").style, {
  zIndex: 405,
  mixBlendMode: "normal",
});

$.getJSON("/Data/KEC_BANYUDONO/Kuwiran.geojson", function (data) {
  DesaKuwiran.addData(data);
});
