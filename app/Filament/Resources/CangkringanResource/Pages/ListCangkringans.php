...
use App\Exports\CangkringanExport;
...
class ListCangkringans extends ListRecords
{
    ...
    {
        return [
            ButtonAction::make('importCangkringan')
                ->label('Import Cangkringan')
                ->action('importCangkringan')
    ...
    {
        return Excel::download(new CangkringanExport, 'cangkringan.xlsx');
    }

    public function downloadGeojson()
    {
        // S3 object key (path to the file in the bucket)
        $filePath = 'pertanian-boyolali/geojson-files/CANGKRINGAN.geojson';
    ...
    public function importCangkringan()
    {
        Artisan::call('import-cangkringan');

        // Optionally, you can return a success notification or redirect
        Notification::make()
            ->title('Import Selesai')
            ->body('Cangkringan berhasil diimport.')
            ->success()
            ->send();
    }
}
