...
use App\Exports\DenggunganExport;
...
class ListDenggungans extends ListRecords
{
    ...
    {
        return [
            ButtonAction::make('importDenggungan')
                ->label('Import Denggungan')
                ->action('importDenggungan')
    ...
    {
        return Excel::download(new DenggunganExport, 'denggungan.xlsx');
    }

    public function downloadGeojson()
    {
        // S3 object key (path to the file in the bucket)
        $filePath = 'pertanian-boyolali/geojson-files/DENGGUNGAN.geojson';

        // The filename to be used for the downloaded file
        $fileName = 'DENGGUNGAN.geojson';
    ...
    public function importDenggungan()
    {
        Artisan::call('import-denggungan');

        // Optionally, you can return a success notification or redirect
        Notification::make()
            ->title('Import Selesai')
            ->body('Denggungan berhasil diimport.')
            ->success()
            ->send();
    }
}
