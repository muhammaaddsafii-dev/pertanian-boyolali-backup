...
class ListKuwrans extends ListRecords
{
    ...
    {
        return [
            ButtonAction::make('importKuwran')
                ->label('Import Kuwran')
                ->action('importKuwran')
    ...
    {
        return Excel::download(new KuwranExport, 'kuwran.xlsx');
    }

    public function downloadGeojson()
    {
        // S3 object key (path to the file in the bucket)
        $filePath = 'pertanian-boyolali/geojson-files/KUWRAN.geojson';

        // The filename to be used for the downloaded file
        $fileName = 'KUWRAN.geojson';
    ...
    public function importKuwran()
    {
        Artisan::call('import-kuwran');

        // Optionally, you can return a success notification or redirect
        Notification::make()
            ->title('Import Selesai')
            ->body('Kuwran berhasil diimport.')
            ->success()
            ->send();
    }
}
